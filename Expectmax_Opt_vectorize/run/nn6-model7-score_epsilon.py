...
def train(model, optimizer, loss, epoch = 0, running_mean = 2048):
    ...
    while epoch != num_epochs:
        ...
        if epoch % 2 == 0:
            ...
            game_len, max_score, game_score, last_loss = gen_sample_and_learn(model, optimizer, loss, True, 0.2)
        ...
        if sum(ls) / 10 > running_mean or epoch % 500 == 0:
            ...
            filename = "model_score_epsilon"+str(name)+".pth.tar"
            save_model({
                'epoch': epoch,
...
print ("Test of model at 2000 epoch")
test(model)

if model1000 is not None:
