...
def gen_sample_and_learn(model, optimizer, loss_fn, is_train = False, explorationProb=0.1):
    ...
    while True:
        ...
        if is_train:
            ...
            y = torch.from_numpy(y).unsqueeze(dim=1).float()
            model.train()
            optimizer.zero_grad()
            pred = model(x)
...
model, optimizer, epoch = load_model(model, optimizer, "/Users/yulianzhou/Desktop/models/4/nodecay/model4-2_2000.pth.tar")
